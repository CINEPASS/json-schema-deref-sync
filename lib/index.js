...
function getRefSchema(refVal, refType, parent, options, state) {
  if (refType && loaders[refType]) {
    var newVal, oldBasePath, loaderValue, filePath, fullRefFilePath;
...
/**
 * Deref schema. file $ref type is done first, then local
 * @param schema
 * @param options
 * @param state
 */
function derefSchema(schema, options, state) {
  ...
  while (!options.keepLocalRefs && !state.circular && hasRefs(schema, state.missing, 'local')) {
    schema = derefType(schema, options, state, 'local');
  }
  
  if (options.keepLocalRefs) {
    traverse(schema).forEach(function (node) {
      if(node && node.definitions) {
        _.extend(definitions, node.definitions);
        this.update(_.omit(node, 'definitions'))
      }
    });
