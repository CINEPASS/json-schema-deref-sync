...
function derefSchema (schema, options, state) {
  ...
  if (state.circular && options.failOnCircular) {
  ...
  return traverse(schema).forEach(function (node) {
    if (!_.isNull(node) && !_.isUndefined(null) && typeof node.$ref === 'string') {
      ...
      if (!addOk) {
        ...
        this.update(JSON.parse(JSON.stringify(node).replace('$ref', '$ref_circular')))
      } else {
 ...
 * @param {Boolean} options.failOnCircular - By default an Error will be returned if the schema contains a circular reference. 
 *                                         If set to <code>false</code> ciruclar references will be resolved. Default: <code>true</code>
...
function deref (schema, options) {
  ...
  // ret = traverse(ret).map(function (x) {
